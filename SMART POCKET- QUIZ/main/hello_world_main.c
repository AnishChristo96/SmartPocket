...
void e_paper_task(void *pvParameter)
{
    ...
    while(1){
        ...
        iot_epaper_draw_string(device, 180, 0, "Kadavul Vazthu", &epaper_font_12, COLORED);
        iot_epaper_draw_string(device, 5, 0, "Kural No:001 ", &epaper_font_16, COLORED);

        iot_epaper_draw_string(device, 15, 25, "Akara Mudhala Ehuth ellam", &epaper_font_12, COLORED);
        iot_epaper_draw_string(device, 15, 35, "Adhibhagavan Mudhatrae Ulagu", &epaper_font_12, COLORED);
       ...
       // sprintf(tsens_str, "%4d C", (int8_t) (esp_random() * 100.0 / UINT32_MAX - 50));
        // iot_epaper_draw_string(device, 170, 50, hum_str, &epaper_font_12, COLORED);
        // iot_epaper_draw_string(device, 170, 80, tsens_str, &epaper_font_12, COLORED);

       //  iot_epaper_draw_horizontal_line(device, 5, 15, 140, COLORED);
       ...
       // iot_epaper_draw_vertical_line(device, 150, 43, 60, COLORED);
        ...
        iot_epaper_draw_string(device, 15, 60, "As the letter A is the first of", &epaper_font_12, COLORED);
        iot_epaper_draw_string(device, 15, 70, "all letters, so the eternal ", &epaper_font_12, COLORED);
        iot_epaper_draw_string(device, 15, 80, "God is first in the world", &epaper_font_12, COLORED);
