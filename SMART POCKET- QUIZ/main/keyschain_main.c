...
#define MISO_PIN -1
#define SCK_PIN 18
#define BUSY_PIN 32 //22 before
#define DC_PIN 27   //21 before
#define RST_PIN 16
#define CS_PIN 19
...
static char *KEY_B3_PREV = "b3_count";
static char *KEY_B4_NEXT = "b4_count";
// static char *KEY_B3_FAV = "b3_count";
static char *KEY_B4_POWER = "b5_count";
...
void e_paper_task(const void *ans_no)
{       
    int ans_no1= (int)ans_no;
    ...
    int ans = 100;
    if (ans_no1==0){
        iot_epaper_draw_string(device, 5, 0, "Welcome", &epaper_font_16, COLORED);
    }
    else{
        
        if(current_kural_number==11 ){
            printer= 9;
        } else {
            printer = current_kural_number - 2;
        }
        char *answer = strNameList[printer][7];
        char *word= strNameList[printer][8];
        ans = atoi (answer);
        if(ans==ans_no1){
            iot_epaper_draw_string(device, 5, 0, "Correct", &epaper_font_16, COLORED);
            iot_epaper_draw_string(device, 8, 25, "Good Job", &epaper_font_12, COLORED);
            iot_epaper_draw_string(device, 8, 35, "!! Congrats !!", &epaper_font_12, COLORED);
        } else {
            iot_epaper_draw_string(device, 5, 0, "Wrong", &epaper_font_16, COLORED);
            iot_epaper_draw_string(device, 8, 25, "Correct Ans is:", &epaper_font_12, COLORED);
            iot_epaper_draw_string(device, 8, 35, word, &epaper_font_12, COLORED);
        }
    }
    ...
    err = nvs_open(STORAGE_NAMESPACE, NVS_READWRITE, &my_handle);
    // Read
    int32_t correctAns = 0; // value will default to 0, if not set yet in NVS
    err = nvs_get_i32(my_handle, "correctAnss", &correctAns);
    ...
    printf("\n%d", correctAns);
    err = nvs_set_i32(my_handle, "correctAnss", correctAns);
    ...
    if(printer==9)
    {
        ...
        int percentAns= ((correctAns*100)/10);
        sprintf(ansStr,"%d%%", percentAns);
        printf("\n\n%d", percentAns);
        if(correctAns<4){
            iot_epaper_draw_string(device, 5, 0, "Do Better !", &epaper_font_16, COLORED);
        }
        if(correctAns<8 && correctAns>3){
            iot_epaper_draw_string(device, 5, 0, "Good !!", &epaper_font_16, COLORED);
        }
        if(correctAns<11 && correctAns>7){
            iot_epaper_draw_string(device, 5, 0, "Excellent !!!", &epaper_font_16, COLORED);
        }
        iot_epaper_draw_string(device, 8, 25, "Your score is:", &epaper_font_16, COLORED);
        iot_epaper_draw_string(device, 8, 35, ansStr, &epaper_font_12, COLORED);
    ...
    current_kural_number = get_i32_in_nvs(KEY_KURAL_NUMBER);
    ...
    printer = current_kural_number - 1;
    // char *a = strNameList[printer][4];
    char *b = strNameList[printer][0];
    char *c = strNameList[printer][1];
    char *d = strNameList[printer][2];
    char *e = strNameList[printer][3];
    // char *f = strNameList[printer][3];
    char *g = strNameList[printer][4];
    char *h = strNameList[printer][5];
    char *i = strNameList[printer][6];
    ...
    iot_epaper_draw_string(device, 8, 25, c, &epaper_font_12, COLORED);
    iot_epaper_draw_string(device, 8, 35, d, &epaper_font_12, COLORED);
    iot_epaper_draw_rectangle(device, 3, 20, 293, 50, COLORED);
...
esp_err_t save_button_press1(void)
{
    ...
    int32_t button_press3 = get_i32_in_nvs(KEY_B3_PREV);
    int32_t button_press4 = get_i32_in_nvs(KEY_B4_NEXT);

    button_press1++;
    set_i32_in_nvs(KEY_B1_PREV, button_press1);

    int totalButtonPress = button_press1 + button_press2 + button_press3 + button_press4;
    char totalchar[500];
...
esp_err_t save_button_press3(void)
{
    ...
    button_press3++;
    set_i32_in_nvs(KEY_B3_PREV, button_press1);
...
esp_err_t save_button_press4(void)
{
    ...
    button_press4++;
    set_i32_in_nvs(KEY_B4_NEXT, button_press1);
...
void app_main(void)
{
    ...
    //Once sample ADC1
        ...
        final= final/1000;
        int percent= (adc_reading*100)/4096;
    ...
    while (1)
    {
        ...
        if (gpio_get_level(GPIO_NUM_26) != 0)
        {
            ...
            if (gpio_get_level(GPIO_NUM_26) == 0)
            {
                ...
                if (err != ESP_OK)
                    printf("Error (%s) saving button press blob to NVS!\n", esp_err_to_name(err));
                xTaskCreate(&e_paper_task, "epaper_task", 4 * 1024, (void *)1, 5, NULL);
        ...
        if (gpio_get_level(GPIO_NUM_13) != 0)
        {
            ...
            if (gpio_get_level(GPIO_NUM_13) == 0)
            {
                ...
                if (err != ESP_OK)
                    printf("Error (%s) saving button press blob to NVS!\n", esp_err_to_name(err));
                xTaskCreate(&e_paper_task, "epaper_task", 4 * 1024, (void *)2, 5, NULL);
        ...
        if (gpio_get_level(GPIO_NUM_2) != 0)
        {
            ...
            if (gpio_get_level(GPIO_NUM_2) == 0)
            {
                ...
                if (err != ESP_OK)
                    printf("Error (%s) saving button press blob to NVS!\n", esp_err_to_name(err));
                xTaskCreate(&e_paper_task, "epaper_task", 4 * 1024, (void *)4, 5, NULL);
        ...
        if (gpio_get_level(GPIO_NUM_14) != 0)
        {
            ...
            if (gpio_get_level(GPIO_NUM_14) == 0)
            {
                ...
                if (err != ESP_OK)
                    printf("Error (%s) saving button press blob to NVS!\n", esp_err_to_name(err));
                xTaskCreate(&e_paper_task, "epaper_task", 4 * 1024, (void *)3, 5, NULL);
