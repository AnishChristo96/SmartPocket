...
#define STORAGE_NAMESPACE_DEFAULT "storage_default"
...
int32_t get_i32_in_nvs(char *key)
{
    ...
    esp_err_t err = nvs_open_from_partition("nvs_1", STORAGE_NAMESPACE, NVS_READWRITE, &my_handle);
    if (err != ESP_OK)
    {
...
int32_t get_i32_in_nvs_default(char *key)
{
    nvs_handle_t my_handle1;
    esp_err_t err = nvs_open(STORAGE_NAMESPACE_DEFAULT, NVS_READWRITE, &my_handle1);
    if (err != ESP_OK)
    {
        ESP_LOGE(TAG, "%s", esp_err_to_name(err));
        return err;
    ...
    int32_t value = 0;
    err = nvs_get_i32(my_handle1, key, &value);
    if (err != ESP_OK && err != ESP_ERR_NVS_NOT_FOUND)
    {
        ESP_LOGE(TAG, "%s", esp_err_to_name(err));
        return err;
...
esp_err_t set_i32_in_nvs_default(char *key, int32_t value)
{
    nvs_handle_t my_handle1;
    esp_err_t err = nvs_open(STORAGE_NAMESPACE_DEFAULT, NVS_READWRITE, &my_handle1);
    if (err != ESP_OK)
    ...
    get_i32_in_nvs_default(key);
    err = nvs_set_i32(my_handle1, key, value);
    if (err != ESP_OK)
    {
        ESP_LOGE(TAG, "%s", esp_err_to_name(err));
        return err;
...
{   
    ...
    nvs_handle_t my_handle1;
    esp_err_t err;
    // err = nvs_open_from_partition("nvs_1", STORAGE_NAMESPACE, NVS_READWRITE, &my_handle);
    
    err = nvs_open(STORAGE_NAMESPACE_DEFAULT, NVS_READWRITE, &my_handle1);

    int32_t current_kural_number = get_i32_in_nvs_default(KEY_KURAL_NUMBER);
    int find_fav = 0;
    size_t fav_size = 0;
    int32_t restart_counter = 0; // value will default to 0, if not set yet in NVS                          //  FOR BATTERY TESTING
    err = nvs_get_i32(my_handle1, "restart_conter", &restart_counter);                                       //  FOR BATTERY TESTING
    sprintf(r, "%d", restart_counter);                    
    // printf("\nThe current iteration is: %d", restart_counter);                                                  //  FOR BATTERY TESTING
    int32_t adc_nvs = 0; // value will default to 0, if not set yet in NVS                          //  FOR BATTERY TESTING
    err = nvs_get_i32(my_handle1, "adc_nnvs", &adc_nvs);                                       //  FOR BATTERY TESTING
    sprintf(s, "%d", adc_nvs);
    // obtain required memory space to store blob being read from NVS
    err = nvs_get_blob(my_handle1, KEY_FAVORITES, NULL, &fav_size);
    if (err != ESP_OK && err != ESP_ERR_NVS_NOT_FOUND)
        return;
    ...
    else
    {
        ...
        err = nvs_get_blob(my_handle1, KEY_FAVORITES, fav_id, &fav_size);
        if (err != ESP_OK)
        {
            free(fav_id);
            return;
    ...
    if (percent2<7){
        iot_epaper_draw_string(device, 8, 53, "     BATTERY VERY LOW", &epaper_font_16, COLORED);
        iot_epaper_draw_string(device, 8, 93, "     PLEASE RECHARGE", &epaper_font_16, COLORED);
    } else if (current_kural_number==0){
        iot_epaper_draw_string(device, 8, 53, "     SMART POCKET", &epaper_font_16, COLORED);
        iot_epaper_draw_string(device, 8, 93, "    KNOWLEDGE IS HERE", &epaper_font_16, COLORED);
    } else {
        lowBatt=0;
        ...
        if(percent2<21 && percent2>7){
        iot_epaper_draw_string(device, 220, 83, "LOW BATTERY", &epaper_font_12, COLORED);
        }
        ...
        // iot_epaper_draw_char(device, 225, 113, "௵", &myfont, COLORED);
...
esp_err_t save_restart_counter(void)
{
    ...
    err = nvs_get_i32(my_handle1, "restart_conter", &restart_counter);
    if (err != ESP_OK && err != ESP_ERR_NVS_NOT_FOUND) return err;

    // Write
    restart_counter++;
    err = nvs_set_i32(my_handle1, "restart_conter", restart_counter);
    if (err != ESP_OK) return err;
...
esp_err_t save_adc(int adc_valf)
{
    ...
    err = nvs_get_i32(my_handle1, "adc_nnvs", &adc_nvs);
    if (err != ESP_OK && err != ESP_ERR_NVS_NOT_FOUND) return err;

    // Write
    adc_nvs=adc_valf;
    err = nvs_set_i32(my_handle1, "adc_nnvs", adc_nvs);
    if (err != ESP_OK) return err;
...
esp_err_t save_button_press1(void)
{
    ...
    set_i32_in_nvs_default(KEY_KURAL_NUMBER, current_kural_number);
...
int save_fav(void)
{
    ...
    int32_t button_press3 = get_i32_in_nvs_default(KEY_B3_FAV);

    button_press3++;
    set_i32_in_nvs_default(KEY_B3_FAV, button_press3);
        ...
        for (int i = 0; i <= (fav_size / sizeof(uint32_t)); i++)
        {
            if (fav_size > i)
            {
                if (fav_id[i] == current_kural_number)
                {
                    ...
                    err = nvs_set_blob(my_handle1, KEY_FAVORITES, fav_id, fav_size);
                    free(fav_id);
                    break;
                }
            }
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_WRITE_EVT:
    {
        ...
        if (!param->write.is_prep)
        {
            ...
            {
            case 48:
                set_i32_in_nvs_default(KEY_READ_OFFSET, atoi(h2s));
                break;
            case 49:
                set_i32_in_nvs_default(KEY_KURAL_NUMBER, atoi(h2s));
            ...
            case 53:
                // print_what_saved();
                printf("This function of printing all Values has been Omitted due to NVS confusion");
                break;
            ...
            case 55:
                ...
                break;
            case 56:
                printf("Deleting NVS_1 Storage: B1,B2,B4,BLE,Kural_Time_Strings");
                nvs_flash_erase_partition("nvs_1");
                vTaskDelay(50 / portTICK_PERIOD_MS);
                break;
            case 57:
                printf("Deleting NVS_Default Storage: Kural_No, Favourites, Favourites_BLOB, char_offset");
                nvs_flash_erase();
                vTaskDelay(50 / portTICK_PERIOD_MS);
                break;
...
static void gatts_profile_b_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_READ_EVT:
    {
        ...
        int runj = 0;
        int read_offset = get_i32_in_nvs_default(KEY_READ_OFFSET);
    ...
    {
        nvs_flash_erase_partition("nvs_1");
        strftime(strftime_buf, sizeof(strftime_buf), "%c", &timeinfo);                              // HOPEFULLY ALL THIS WILL PUT IT TO SLEEP AFTER BT DISCONNECT... IT DID !!
        ESP_LOGI(TAG2, "The current date/time in India is: %s", strftime_buf);
        vTaskDelay(50 / portTICK_PERIOD_MS);
        const int wakeup_time_sec = 1800;
        uint64_t sleeptime = wakeup_time_sec * 1000000;
...
void app_main(void)
{
    esp_err_t err = nvs_flash_init_partition("nvs_1");
    if (err == ESP_ERR_NVS_NO_FREE_PAGES || err == ESP_ERR_NVS_NEW_VERSION_FOUND)
    {
        // NVS partition was truncated and needs to be erased
        ESP_ERROR_CHECK(nvs_flash_erase_partition("nvs_1"));
        vTaskDelay(50 / portTICK_PERIOD_MS);
        nvs_flash_erase_partition("nvs_1");
        vTaskDelay(50 / portTICK_PERIOD_MS);
        // Retry nvs_flash_init
        err = nvs_flash_init_partition("nvs_1");
    }
    ESP_ERROR_CHECK(err);
    ...
    //Once sample ADC1
        ...
        err = save_adc(adc_reading);                                                                        //  FOR BATTERY TESTING
                if (err != ESP_OK)                                                                          //  FOR BATTERY TESTING
                    printf("Error (%s) saving ADC Value to NVS!\n", esp_err_to_name(err));          //  FOR BATTERY TESTING

        float final= (adc_reading *1.02539);
        final= final/1000;
        // int percent= (adc_reading*100)/4095;
        int bat_current= 3920-adc_reading;
        if (bat_current<1){bat_current=0;}
        int bat_perc_used=(bat_current*100)/870;  // SETTING 3920 AS MAX AND 3050 AS MIN ADC VALUES FOR 0 TO 100 % (870 IS DIFFERENCE)
        int percent= 100-bat_perc_used;
    ...
    case ESP_SLEEP_WAKEUP_EXT1:
    {
        ...
        if (wakeup_pin_mask != 0)
        {
            ...
            if (err != ESP_OK)
                printf("Error (%s) saving button press B4_POWER to NVS!\n", esp_err_to_name(err));
        }
        else
        {
    ...
    {
        printf("Wake up from timer. Time spent in deep sleep: %ldms\n", sleep_time_ms);
    ...
    if (get_i32_in_nvs_default(KEY_KURAL_NUMBER) == 0)
    {
        // set_i32_in_nvs(KEY_KURAL_NUMBER, 1);
        if (pin==100){
            xTaskCreate(&e_paper_task, "epaper_task", 4 * 1024, (void *)bar, 5, NULL);   //YET TO DO: WELCOME SCREEN.... DONE !!
        }
    }
 ...
 b2++;
                ...
                if (err != ESP_OK)
                    printf("Error (%s) saving button press 2-NEXT to NVS!\n", esp_err_to_name(err));
    ...
    while (1)
    {
        ...
        if (gpio_get_level(GPIO_NUM_26) != 0 || pin==26)
        {
            ...
            {   
                ...
                if (err != ESP_OK)
                    printf("Error (%s) saving button press 1-PREV to NVS!\n", esp_err_to_name(err));
                }
        ...
        if (gpio_get_level(GPIO_NUM_2) != 0)
        {
            ...
            if (gpio_get_level(GPIO_NUM_2) != 0)
            {
                if (percent > 7){
                    ...
                    if (err != ESP_OK)
                        printf("Error (%s) saving button press BLE_MODE to NVS!\n", esp_err_to_name(err));
                    ...
                    err= nvs_flash_erase_partition("nvs_1");
                    break;
                }
            }
        }
