...
{   
    int percent3=0;                             // LOGIC WAS USED HERE WAS FOR TURNING OFF TIME & DATE ON SLEEP WHICH IS 'NOW NOT IN USE'
...
esp_err_t save_button_press1(void)
{
    ...
    set_i32_in_nvs(KEY_KURAL_NUMBER, current_kural_number);
                // COMMENTING OUT FOR BATTERY TESTING BECAUSE NVS KEEPS GETTING FULL
    ...
    if (err != ESP_OK)
        return err;
                // END OF COMMENTING FOR BATTERY TESTING
    err = nvs_commit(my_handle);
    if (err != ESP_OK)
        return err;
...
void app_main(void)
{
    ...
    case ESP_SLEEP_WAKEUP_EXT1:
    {
        ...
        if (wakeup_pin_mask != 0)
        {
            ...
            if (err != ESP_OK)
                printf("Error (%s) saving button press B4_POWERR to NVS!\n", esp_err_to_name(err));
        }
        else
        {
    ...
    while (1)
    {
        ...
        if (gpio_get_level(GPIO_NUM_2) != 0)
        {
            ...
            if (gpio_get_level(GPIO_NUM_2) == 0)
            {
                err = increment_nvs_value(KEY_B4_POWER);
                if (err != ESP_OK){
                    printf("Error (%s) saving button press B4-POWER to NVS!\n", esp_err_to_name(err));
                }
                bar=bar+1000;
                xTaskCreate(&e_paper_task, "epaper_task", 4 * 1024, (void *)bar, 5, NULL);
                // vTaskDelay(200 / portTICK_PERIOD_MS);
        ...
        if (gpio_get_level(GPIO_NUM_2) != 0)
        {
            ...
            if (gpio_get_level(GPIO_NUM_2) != 0)
            {
                ...
                if (err != ESP_OK)
                    printf("Error (%s) saving button press BLE-MODE to NVS!\n", esp_err_to_name(err));
    ...
    ESP_LOGI(TAG2, "The current date/time in India is: %s", strftime_buf);
    // vTaskDelay(2500 / portTICK_PERIOD_MS);
    const int wakeup_time_sec = 2;                                          // FOR BATTERY TESTING
